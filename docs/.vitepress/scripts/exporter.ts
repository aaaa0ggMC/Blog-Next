...
export function getPagePlainText(rootEl?: Element | null): string {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (tagName === 'li') {
        ...
        for (const child of Array.from(el.childNodes)) {
          childText += processTextNode(child)
        }
      ...
      let innerText = ''
      for (const child of Array.from(el.childNodes)) {
        innerText += processTextNode(child)
      }
  ...
  let text = processTextNode(content)
...
export function getPageMarkdown(rootEl?: Element | null): string {
  ...
  function processNode(node: Node, level = 0, inParagraph = false): string {
    if (shouldIgnore(node)) return ''
    ...
    if (tagName === 'strong' || tagName === 'b') {
      let inner = ''
      for (const child of Array.from(el.childNodes)) {
        inner += processNode(child, level, inParagraph)
      }
      return `**${inner.trim()}**`
    }
    ...
    if (tagName === 'a') {
      ...
      let text = ''
      for (const child of Array.from(el.childNodes)) {
        text += processNode(child, level, inParagraph)
      }
      text = text.trim()
    ...
    if (tagName === 'ul' || tagName === 'ol') {
      ...
      for (const child of Array.from(el.childNodes)) {
        if (child.nodeType === Node.ELEMENT_NODE && (child as HTMLElement).tagName.toUpperCase() === 'LI') {
          listContent += processNode(child, level)
        }
      }
      return `\n\n${listContent}\n`
    }
            ...
            for (const child of Array.from(cell.childNodes)) {
              if (child.nodeType === Node.TEXT_NODE) {
                ...
                if (childEl.tagName.toLowerCase() === 'br') {
                  ...
                  cellText += processNode(childEl, level, true)
                }
              }
            }
  ...
  let formatted = processNode(content)
